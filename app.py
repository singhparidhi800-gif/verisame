...
PRO_AMOUNT = 2999
WAIT_SECONDS = 15 # 15 second wait - fast but still friction
ADMIN_KEY = "Reyansh" # TERA SECRET PASSWORD
...
if 'free_clicks' not in st.session_state:
    st.session_state.free_clicks = 0
if 'pro_clicks' not in st.session_state:
    st.session_state.pro_clicks = 0
if 'pro_unlocked' not in st.session_state:
    st.session_state.pro_unlocked = 0
if 'counted_visit' not in st.session_state:
    st.session_state.counted_visit = False

# Count visit once per session
if not st.session_state.counted_visit:
...
if 'admin' in query_params and query_params['admin'] == ADMIN_KEY:
    st.title("🔐 SECRET ADMIN DASHBOARD")
    st.caption("Sirf tere liye. Kisi aur ko ye nahi dikhega.")
    st.markdown("---")

    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("👥 Total Visitors", st.session_state.total_visits)
    with col2:
        st.metric("🆓 FREE Clicks", st.session_state.free_clicks)
    with col3:
        st.metric("💎 PRO Clicks", st.session_state.pro_clicks)
    with col4:
        st.metric("✅ PRO Unlocked", st.session_state.pro_unlocked)

    st.markdown("---")
    conversion = (st.session_state.pro_unlocked / st.session_state.pro_clicks * 100) if st.session_state.pro_clicks > 0 else 0
    st.metric("📈 PRO Conversion Rate", f"{conversion:.1f}%")
    st.metric("💰 Estimated Revenue", f"₹{st.session_state.pro_unlocked * PRO_AMOUNT}")
    ...
    if st.button("🔄 Reset All Stats", type="secondary"):
        ...
        st.session_state.pro_clicks = 0
        st.session_state.pro_unlocked = 0
        st.success("Stats reset ho gaye")
        st.rerun()
...
if st.session_state.plan is None:
    ...
    with col1:
        ...
        if st.button("Use FREE", use_container_width=True):
            st.session_state.free_clicks += 1
        ...
        if is_pro:
            ...
            if not st.session_state.payment_done:
                ...
                # STAGE 2: QR CODE + HIDDEN TIMER
                    ...
                    # ============ HIDDEN TIMER LOGIC WITH PROGRESS BAR ============
                    ...
                    if elapsed_time < WAIT_SECONDS:
                        ...
                        with col1:
                            if st.button("🔓 Unlock Download Now", use_container_width=True, type="primary"):
                                ...
                                st.session_state.pro_unlocked += 1
                                st.session_state.show_qr = False
                                st.balloons()
                                st.rerun()
                        with col2:
