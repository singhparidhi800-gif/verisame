...
# FIX: Cache hata diya taaki har user ko fresh data mile
def save_db(d):
...
T = {
    ...
    "upi_text":"Scan QR to Pay","paid_btn":"Customer I Paid ₹{amount}","success_msg":"Payment Submitted! Awaiting Admin Approval",
    "download_success":"Download Ready!","locked":"Upgrade to Pro for all tools","tab1":"Date & Nulls","tab2":"Email & Phone","tab3":"Text Tools",
    ...
    "expired":"Plan Expired","admin_title":"Sherni Admin Panel","admin_pending":"Pending Approvals","admin_approve_btn":"Mark Paid & Approve",
    "admin_user":"Customer Email","admin_plan":"Plan","admin_expiry":"Valid Till","admin_status":"Status","download_btn":"Download Now",
    "delete_btn":"Delete User","pro_lock_msg":"🔒 Get Pro plan for all tools"
}

# ========== RESPONSIVE + MOBILE FIX CSS ==========
st.markdown("""
<style>
...
.stMarkdown p {color: #374151!important; margin-bottom: 0.3rem!important; font-size: 0.95rem!important;}

.pricing-card {border: 2px solid rgba(236,72,153,0.3); border-radius: 22px; padding: 1.6rem; background: rgba(255,255,255,0.95); backdrop-filter: blur(12px); transition: all 0.3s ease; box-shadow: 0 10px 25px rgba(124,58,237,0.12); height: 100%;}
...
.stButton>button {border-radius: 14px; font-weight: 600; background: linear-gradient(90deg, #8B5CF6, #EC4899); color: white!important; border: none; padding: 13px 26px; width: 100%; box-shadow: 0 5px 18px rgba(139,92,246,0.45); transition: all 0.3s; cursor: pointer; font-size: 1rem!important;}
...
/* FIX: All text visible on mobile */
label,.stMultiSelect label,.stSelectbox label,.stTextInput label {color: #111827!important; font-weight: 600!important; font-size: 0.95rem!important;}
div[data-baseweb="select"] div, div[data-baseweb="select"] input {color: #111827!important; font-size: 0.95rem!important;}
.stAlert {color: #111827!important; font-weight: 500!important; background: rgba(255,255,255,0.95)!important; border-radius: 12px;}
.stInfo {background: rgba(219,234,254,0.95)!important; border-left: 4px solid #3B82F6!important;}
.stSuccess {background: rgba(209,250,229,0.95)!important; border-left: 4px solid #10B981!important;}
.stError {background: rgba(254,226,226,0.95)!important; border-left: 4px solid #EF4444!important;}

.pro-banner {background: linear-gradient(135deg, rgba(124,58,237,0.95), rgba(236,72,153,0.95)); backdrop-filter: blur(18px); padding: 1.6rem; border-radius: 22px; color: white!important; text-align: center; margin: 1rem 0; border: 1.5px solid rgba(255,255,255,0.35);}
.tool-chip {display: inline-block; background: rgba(255,255,255,0.25); backdrop-filter: blur(12px); padding: 9px 17px; border-radius: 28px; margin: 4px; font-weight: 600; border: 1.5px solid rgba(255,255,255,0.4); color: white!important; font-size: 0.92rem;}
...
.admin-card {background: rgba(255,255,255,0.98); padding: 1.3rem; border-radius: 18px; margin: 0.7rem 0; border: 2px solid rgba(236,72,153,0.25); box-shadow: 0 5px 15px rgba(124,58,237,0.1); color: #111827!important;}
.element-container {margin-bottom: 0.35rem!important;}
.stTextInput input {border-radius: 12px; border: 2px solid #D8B4FE; color: #111827!important; background: white!important;}

/* Tabs visible */
div[data-testid="stTabs"] button p {color: white!important; font-weight: 600!important; font-size: 1rem!important;}
div[data-testid="stTabs"] button[aria-selected="true"] p {color: #FFD700!important; font-weight: 800!important; border-bottom: 3px solid #FFD700;}
div[data-testid="stTabs"] button {background: rgba(255,255,255,0.25)!important; border-radius: 12px; margin-right: 8px;}
...
with col3:
    st.markdown("""<div style="width: 100%; min-height: 240px;"><img src="https://i.postimg.cc/8zdnX54g/IMG-20260609-WA0012.jpg" style="width: 100%; height: 240px; object-fit: cover; object-position: center top; border-radius: 25px; box-shadow: 0 15px 35px rgba(236,72,153,0.6);"></div>""", unsafe_allow_html=True)
                ...
                with col3:
                    if st.button(T['admin_approve_btn'], key=f"verify_{email}", type="primary", use_container_width=True):
                        if admin_email_input and "@" in admin_email_input:
                            ...
                            save_db(data)
                            customer_link = f"https://verisame.streamlit.app/?email={admin_email_input}"
                            st.success(f"✓ {admin_email_input} Marked Paid!")
                            st.code(customer_link)
                            st.markdown(f"**[Send this link to customer]({customer_link})**")
                            st.balloons()
                            st.rerun()
                        else:
                            st.error("Enter valid email")
                with col4:
                    if st.button(T['delete_btn'], key=f"del_pending_{email}", use_container_width=True):
                        del data[email]
                        save_db(data)
                        st.rerun()
        st.markdown("---")
        ...
        with col_free:
            ...
            for email,info in all_users.items():
                if info.get('plan')=='free':
                    ...
                    if st.button(T['delete_btn'], key=f"del_free_{email}", use_container_width=True):
                        del data[email]
                        save_db(data)
                        st.rerun()
        with col_pro299:
            ...
            for email,info in all_users.items():
                if info.get('plan')=='pro' and info.get('amt')==299:
                    ...
                    if st.button(T['delete_btn'], key=f"del_299_{email}", use_container_width=True):
                        del data[email]
                        save_db(data)
                        st.rerun()
        with col_pro1499:
            ...
            for email,info in all_users.items():
                if info.get('plan')=='pro' and info.get('amt')==1499:
                    status_color = "#7C3AED" if info.get('status')=="PAID" else "#EC4899"
                    st.markdown(f"<div class='admin-card'><b>{email}</b><br>Valid: {info.get('expiry','N/A')}<br>Status: <span style='color:{status_color};font-weight:600'>{info.get('status','N/A')}</span></div>", unsafe_allow_html=True)
                    if st.button(T['delete_btn'], key=f"del_1499_{email}", use_container_width=True):
                        del data[email]
                        save_db(data)
                        st.rerun()
        st.stop()
...
if st.session_state.plan is None:
    ...
    if df is not None:
        ...
        with tab1:
            ...
            if not is_pro: st.info(T['pro_lock_msg'])
            if st.button(T['apply_btn'], key="btn_date", disabled=not is_pro, use_container_width=True):
            ...
            if not is_pro: st.info(T['pro_lock_msg'])
            if st.button(T['apply_btn'], key="btn_fill", disabled=not is_pro, use_container_width=True):
        ...
        with tab2:
            ...
            if not is_pro: st.info(T['pro_lock_msg'])
            if st.button(T['apply_btn'], key="btn_email", disabled=not is_pro, use_container_width=True):
            ...
            if not is_pro: st.info(T['pro_lock_msg'])
            if st.button(T['apply_btn'], key="btn_phone", disabled=not is_pro, use_container_width=True):
        ...
        with tab3:
            ...
            if not is_pro: st.info(T['pro_lock_msg'])
            if st.button(T['apply_btn'], key="btn_spec", disabled=not is_pro, use_container_width=True):
                for col in spec_cols:
                    st.session_state.df_clean[col] = st.session_state.df_clean[col].str.replace(r'[^a-zA-Z0-9\s@.]', '', regex=True)
                st.success(T['success'])
                st.rerun()
            st.write(f"**{T['tool7']}**")
            old = st.selectbox("Old column name", all_cols, key="sel_old", disabled=not is_pro)
            new = st.text_input("New column name", key="inp_new", disabled=not is_pro)
            if not is_pro: st.info(T['pro_lock_msg'])
            if st.button(T['apply_btn'], key="btn_rename", disabled=not is_pro, use_container_width=True) and new:
                st.session_state.df_clean.rename(columns={old: new}, inplace=True)
                st.success(T['success'])
                st.rerun()
            st.write(f"**{T['tool8']}**")
            if st.button(T['apply_btn'], key="btn_dedup", disabled=not is_pro, use_container_width=True):
            ...
            if not is_pro: st.info(T['pro_lock_msg'])
            if st.button(T['apply_btn'], key="btn_spell", disabled=not is_pro, use_container_width=True):
