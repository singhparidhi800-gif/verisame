...
.stApp {
    background: radial-gradient(circle at 20% 50%, rgba(255,20,147,0.3) 0%, transparent 50%),
                radial-gradient(circle at 80% 50%, rgba(138,43,226,0.3) 0%, transparent 50%),
...
h1 {
    ...
    background: linear-gradient(90deg, #FFFFFF, #FFB6C1, #E0AAFF, #FF69B4, #FFFFFF);
...
@keyframes textGlow {
    ...
    50% {filter: brightness(1.4) drop-shadow(0 0 40px rgba(255,182,193,0.8));}
}
...
[data-testid="stImage"] img {
    ...
    filter: drop-shadow(0 0 40px rgba(255,20,147,0.9));
...
.expiry-alert {
    background: linear-gradient(90deg, #FF0000, #FF1493, #FF0000);
    ...
    animation: redAlert 1s ease-in-out infinite, gradientShift 2s ease infinite;
    border: 3px solid #FFD700;
}

@keyframes redAlert {
    0%,100% {transform: scale(1); box-shadow: 0 0 40px rgba(255,0,0,0.8);}
    50% {transform: scale(1.05); box-shadow: 0 0 60px rgba(255,0,0,1);}
}
...
@keyframes bannerPulse {
    ...
    50% {box-shadow: 0 0 140px rgba(255,20,147,0.9);}
}
...
@keyframes card3D {
    ...
    50% {transform: perspective(1000px) rotateX(0deg) translateZ(25px);}
}
...
@keyframes buttonGlow {
    0%,100% {box-shadow: 0 8px 30px rgba(255,20,147,0.7);}
    50% {box-shadow: 0 20px 50px rgba(255,20,147,1);}
}
...
# SIDEBAR ME EXPIRY DATE + 5 DIN RED ALERT
...
if st.query_params.get("admin") == ADMIN_PASS:
    ...
    for email in all_users:
        ...
        st.markdown(f"<div style='background:rgba(40,0,80,0.9);padding:20px;border-radius:20px;margin:10px 0;border:2px solid #FF1493;color:white'>{badge} <b>Email:</b> {email} | <b>Plan:</b> {plan_text} | <b>Price:</b> {price_text} | <b>Expiry:</b> {info.get('expiry','N/A')} | <b>Status:</b> {info.get('status','N/A')}</div>", unsafe_allow_html=True)
    st.markdown("---")
    ...
    for email,info in data.items():
        if info.get("status")=="PENDING" and "@" in email:
            ...
            c2.markdown(f"<div style='background:rgba(40,0,80,0.9);padding:20px;border-radius:20px;color:white'><b>Plan:</b> {plan_text} | <b>{price_text}</b> | <b>Exp:</b> {info['expiry']}</div>", unsafe_allow_html=True)
...
if st.session_state.plan is None:
    ...
    if df is not None:
        ...
        # PRO PENDING - QR -> I PAID -> ADMIN APPROVE -> DOWNLOAD
            ...
            # I PAID KE BAAD BHI DOWNLOAD NAHI JAB TAK ADMIN APPROVE NA KARE
